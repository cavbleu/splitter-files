...
type FileProcessor interface {
	Process(data []byte, outputDir string, counter int32, startPos int) (int, int, string, string, *OfficeDocumentInfo, error)
}

// DefaultFileProcessor implements the basic file processing
type DefaultFileProcessor struct{}

func (p *DefaultFileProcessor) Process(data []byte, outputDir string, counter int32, startPos int) (int, int, string, string, *OfficeDocumentInfo, error) {
	return extractFile(data, outputDir, counter, startPos)
}
...
type ProcessingStrategy interface {
	Execute(data []byte, outputDir string, counter int32, startPos int) (int, int, string, string, *OfficeDocumentInfo, error)
}
...
func (s *DefaultProcessingStrategy) Execute(data []byte, outputDir string, counter int32, startPos int) (int, int, string, string, *OfficeDocumentInfo, error) {
	return s.processor.Process(data, outputDir, counter, startPos)
}
...
func (wp *WorkerPool) Start(outputDir string) {
	for i := 0; i < wp.numWorkers; i++ {
		wp.wg.Add(1)
		go worker(i, wp.jobs, wp.results, outputDir, wp.wg)
	}
}
...
func validateJpegImproved(data []byte) bool {
	...
	// // Validate known marker segments
	// switch marker {
	// case 0xC0, 0xC1, 0xC2, 0xC3, 0xC5, 0xC6, 0xC7, 0xC9, 0xCA, 0xCB, 0xCD, 0xCE, 0xCF: // SOFn
	// 	if length < 8 {
	// 		return false
	// 	}
	// case 0xE0, 0xE1, 0xE2, 0xE3, 0xE4, 0xE5, 0xE6, 0xE7, 0xE8, 0xE9, 0xEA, 0xEB, 0xEC, 0xED, 0xEE, 0xEF: // APPn
	// 	if length < 2 {
	// 		return false
	// 	}
	// }
	...
	case "jpg", "jpeg":
		...
		for i := len(data) - 2; i >= 0; i-- {
			if data[i] == 0xFF && data[i+1] == 0xD9 { // EOI marker
				fileEnd = i + 2
				// // Check for padding after EOI
				// for j := fileEnd; j < len(data); j++ {
				// 	if data[j] != 0x00 {
				// 		break
				// 	}
				// 	fileEnd++
...
func worker(id int, jobs <-chan FileChunk, results chan<- ExtractionResult, outputDir string, wg *sync.WaitGroup) {
	...
	// Using Strategy pattern for processing
	processor := &DefaultFileProcessor{}
	strategy := &DefaultProcessingStrategy{processor: processor}

	for chunk := range jobs {
		size, endPos, filename, fileType, officeInfo, err := strategy.Execute(chunk.Data, outputDir, chunk.Counter, chunk.Start)
		if err != nil {
...
func main() {
	...
	if len(flag.Args()) < 2 {
		fmt.Println("Usage: file_splitter [flags] <input_file> <output_directory> [num_workers]")
		fmt.Println("Flags:")
		flag.PrintDefaults()
